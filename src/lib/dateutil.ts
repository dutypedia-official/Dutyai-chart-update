...
export function toUTCStamp(date_str: string): number{
  ...
  if(isNumOnly){
    ...
    if(numLen == 4){
      // For MMDD format, use current year
      const currentYear = new Date().getFullYear();
      result = dayjs(`${currentYear}${date_str}`, 'YYYYMMDD')
    }
    else if(numLen == 6){
      // For YYMMDD format, convert to full year
      const year = parseInt(date_str.substring(0, 2));
      const fullYear = year > 50 ? 1900 + year : 2000 + year;
      const monthDay = date_str.substring(2);
      result = dayjs(`${fullYear}${monthDay}`, 'YYYYMMDD')
    }
    else if(numLen == 8){
  ...
  // Validate the result to prevent "Invalid time value" errors
  if(!result || !result.isValid()){
    console.error('Invalid date format or value:', date_str)
    return 0;
  }

<script lang="ts">
  ...
  // Create wrapper functions for chart methods that need extra behavior
  ...
  let originalCreateIndicator: ((indicator: any, isStack?: boolean, paneOptions?: any) => any) | null = null;
  ...
  $effect(() => {
    if ($chart && $chart !== originalChart) {
      ...
      originalCreateIndicator = ($chart as any).createIndicator?.bind($chart) ?? null;
      ...
      // Wrap createIndicator to slightly increase height of new sub‑panes
      if (originalCreateIndicator) {
        ($chart as any).createIndicator = function(indicator: any, isStack?: boolean, paneOptions?: any) {
          const chartAny: any = this;

          // Capture pane list before creating the indicator
          let beforePanes: any[] = [];
          try {
            const res = chartAny.getPaneOptions?.();
            if (Array.isArray(res)) {
              beforePanes = res;
            }
          } catch {
            // ignore, we will just skip before/after diffing
          }

          const result = originalCreateIndicator!(indicator, isStack as any, paneOptions);

          // Capture pane list after creating the indicator
          let afterPanes: any[] = [];
          try {
            const res = chartAny.getPaneOptions?.();
            if (Array.isArray(res)) {
          ...
          try {
            const beforeIds = new Set(beforePanes.map(p => p?.id).filter(Boolean));

            for (const pane of afterPanes) {
              const paneId: string | undefined = pane?.id;
              if (!paneId) continue;
              // Only care about non‑main, non‑x‑axis panes
              if (paneId === 'candle_pane' || paneId === 'x_axis_pane') continue;
              // If this pane didn't exist before, it's a newly created sub‑pane
              if (!beforeIds.has(paneId)) {
                // Desktop only: bump new sub‑pane height; keep mobile default
                if (!isMobileDevice()) {
                  const baseHeight = typeof pane.height === 'number' && pane.height > 0 ? pane.height : undefined;
                  if (baseHeight !== undefined) {
                    const newHeight = baseHeight + 100; // +100px extra height for new sub pane (desktop)
                    try {
                      chartAny.setPaneOptions?.({ id: paneId, height: newHeight });
                      console.log('📏 Increased new sub‑pane height by 100px (desktop only)', { paneId, baseHeight, newHeight });
                    } catch {
                      // ignore – non‑fatal
                    }
                  }
            ...
            // Swallow any safety issues so we don't break indicator creation
          }
  ...
  /**
   * Toggle a pane between normal and maximized state.
   * Uses klinecharts internal pane state handling so that:
   * - state 'maximize' hides all other panes and makes this one full height
   * - state 'normal' restores all panes and previous heights
   */
  function togglePaneState(paneId: string) {
    if (!$chart) return;
    try {
      const chartObj: any = $chart;
      const opts = chartObj.getPaneOptions?.(paneId) || null;
      const currentState: string = opts?.state || 'normal';
      const nextState: 'normal' | 'maximize' =
        currentState === 'maximize' ? 'normal' : 'maximize';
      chartObj.setPaneOptions?.({ id: paneId, state: nextState });
      console.log('🖱️ Pane double-click toggle', { paneId, from: currentState, to: nextState });
    } catch (error) {
      console.warn('⚠️ Failed to toggle pane state', paneId, error);
    }
  }

  /**
   * Desktop: handle double‑click anywhere inside the chart widget.
   *
   * Behavior (TradingView style):
   * - Double‑click on main chart region → maximize `candle_pane` and hide all sub panes.
   * - Double‑click on any sub‑pane region → maximize that sub pane full height.
   * - Double‑click again on the maximized pane → restore all panes to previous layout.
   */
  function handlePaneDoubleClick(e: MouseEvent) {
    if (!$chart || !chartRef) return;

    const chartObj: any = $chart;

    // IMPORTANT:
    // Only enable TradingView-style maximize when there is at least ONE real sub‑pane
    // (an indicator living on its own pane, not on `candle_pane`).
    // If there is no sub‑pane yet, double‑click should do NOTHING so that
    // future sub‑panes are not affected by an old maximize state.
    try {
      const indicators = (chartObj.getIndicators?.() ?? []) as Array<{ paneId?: string }>;
      const hasSubPaneIndicators = indicators.some(ind => ind.paneId && ind.paneId !== 'candle_pane');
      if (!hasSubPaneIndicators) {
        return;
      }
    } catch {
      // If anything goes wrong while checking, fail safe: do nothing.
      return;
    }

    const drawPanes: any[] | undefined = chartObj._drawPanes;

    // If we can't introspect panes, fall back to toggling main candle pane
    if (!Array.isArray(drawPanes) || drawPanes.length === 0) {
      togglePaneState('candle_pane');
      return;
    }
    ...
    for (const pane of drawPanes) {
      if (!pane || typeof pane.getId !== 'function' || typeof pane.getBounding !== 'function') {
        continue;
      }

      const paneId = pane.getId();
      // Ignore the X axis pane – we only care about candle + indicator panes
      if (paneId === 'x_axis_pane') continue;

      const bounding = pane.getBounding();
      if (!bounding || typeof bounding.height !== 'number') continue;

      const top = typeof bounding.top === 'number' ? bounding.top : 0;
    ...
    // If we couldn't map the Y position to a pane, default to main candle pane
    if (!targetPaneId) {
      targetPaneId = 'candle_pane';
    }

    togglePaneState(targetPaneId);
  }

...
let showAIModal = $state(false);
  ...
  let isRotating = $state(false); // Loading state for rotate button
  ...
  async function toggleRotate() {
    if (!browser || isRotating) return; // Prevent multiple clicks during rotation
    
    isRotating = true; // Start loading animation
    ...
    try {
        ...
        if (isIOS()) {
          ...
          try {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                // Stop loading animation immediately after resize
                setTimeout(() => {
                  isRotating = false;
                }, 50);
              }, 50);
            ...
            isRotated = true;
            isRotating = false; // Stop loading animation
          }
        } else {
          ...
          setTimeout(async () => {
            ...
            if ($chart) {
              ...
              setTimeout(() => {
                isRotating = false; // Stop loading animation if no chart
              }, 50);
            }
          }, 50);
        }
      ...
      isRotated = false;
      isRotating = false; // Stop loading animation on error
    }
  }
...
<div bind:this={menuContainerRef} class="menu-container">
  ...
  <div class="menu-left-section">
    ...
    <!-- Rotate button - Mobile/Tablet only, before symbol search -->
    <div class="rotate-button-wrapper-mobile">
      <button 
         class="menu-btn {isRotated ? 'menu-btn-active' : ''} {isRotating ? 'rotate-loading' : ''}" 
       ...
       >
        {#if isRotating}
          ...
          <div class="rotate-loading-overlay">
            <div class="rotate-loading-spinner">
              <div class="rotate-spinner-ring"></div>
              <div class="rotate-spinner-ring"></div>
              <div class="rotate-spinner-ring"></div>
            </div>
            <div class="rotate-loading-text">Rotating...</div>
          </div>
        {/if}
        <div class="menu-btn-content" class:rotate-content-hidden={isRotating}>
          <div class="menu-icon-wrapper">
            <svg width="20" height="20" viewBox="0 0 448 448" fill="none" xmlns="http://www.w3.org/2000/svg" class:rotate-icon-spinning={isRotating}>
              {#if isRotated}
...
/* Rotate Button Styles - Mobile/Tablet on Left Side */
.rotate-button-wrapper-mobile {
  display: none; /* Hidden by default on desktop */
}

/* Show rotate button only on mobile and tablet devices, on left side */
...
/* Old rotate button wrapper - keep for backward compatibility but hide */
.rotate-button-wrapper {
  display: none;
}

/* Rotate Button Loading State */
.rotate-loading {
  position: relative;
  overflow: hidden;
...
.rotate-spinner-ring:nth-child(2) {
  width: 80%;
  height: 80%;
  top: 10%;
  left: 10%;
...
.rotate-spinner-ring:nth-child(3) {
  width: 60%;
  height: 60%;
  top: 20%;
  left: 20%;
...
.rotate-icon-spinning {
  animation: rotate-icon-spin 0.8s linear infinite;
  opacity: 0.5;
}

.rotate-content-hidden {
  opacity: 0.3;
  pointer-events: none;
}
...
/* Light Mode Styles for Rotate Loading */
:global([data-theme="light"]) .rotate-loading-overlay {
  background: rgba(59, 130, 246, 0.12);
}

:global([data-theme="light"]) .rotate-spinner-ring {
  border-top-color: rgba(59, 130, 246, 0.8);
  border-right-color: rgba(59, 130, 246, 0.8);
}

:global([data-theme="light"]) .rotate-spinner-ring:nth-child(2) {
  border-top-color: rgba(139, 92, 246, 0.8);
  border-right-color: rgba(139, 92, 246, 0.8);
}

:global([data-theme="light"]) .rotate-spinner-ring:nth-child(3) {
  border-top-color: rgba(59, 130, 246, 0.6);
  border-right-color: rgba(59, 130, 246, 0.6);
}

:global([data-theme="light"]) .rotate-loading-text {
  color: rgba(30, 41, 59, 0.8);
}

/* Additional mobile-specific styles for rotate button */
@media (max-width: 768px) {
  .rotate-button-wrapper-mobile .menu-btn {
    height: 36px;
    padding: 0 12px;
  }
}

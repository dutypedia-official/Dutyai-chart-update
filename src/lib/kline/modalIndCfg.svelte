<script lang="ts">
  ...
  let volColorPaletteType = $state<'histogram' | 'ema'>('histogram');
      ...
      volGroups = volEntries.map(([saveKey, savedInd]) => {
        ...
        if ((savedInd as any).volGroups && (savedInd as any).volGroups.length > 0) {
          ...
          return {
            ...
            styles: {
              histogram: {upColor: '#26a69a', downColor: '#ef5350', thickness: 2, lineStyle: 'solid'}, // Green/Red
              ema: {color: '#8B5CF6', thickness: 2, lineStyle: 'solid'} // Purple
            }
          };
        }
      });

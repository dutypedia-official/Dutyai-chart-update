<script lang="ts">
  ...
  // Type-safe mapping for timespan message functions
  function getTimespanMessage(timespan: string): string {
    switch (timespan) {
  ...
  <div class="p-2">
    <ul class="flex flex-wrap justify-center gap-2">
      {#each periods as period}
        ...
        >
          <span>{period.multiplier} {getTimespanMessage(period.timespan)}</span>
        </li>
      {/each}
    </ul>
  </div>

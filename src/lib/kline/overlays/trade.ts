...
const trade: OverlayTemplate = {
  ...
  createPointFigures: ({coordinates, overlay}) => {
    ...
    const line_styles: Record<string, unknown> = {size: 0.5}
    ...
    figures.push(line_fig)
    
    if(!data.active && !data.selected) return figures
    ...
    return [hit_area_fig, line_fig, in_fig, out_fig, vert_start, vert_stop]
  },
  ...
  onMouseEnter: (e: OverlayEvent<unknown>) => {
    const data = e.overlay.extendData as TradeInfo;
    data.active = true
    return false
  },
  onMouseLeave: (e: OverlayEvent<unknown>) => {
    const data = e.overlay.extendData as TradeInfo;
    data.active = false
    return false
  }

...
export function getPrecision(chart: Chart, overlay: Overlay, yAxis: kc.Nullable<YAxis>): number {
  // Default precision for price display
  let precision = 2
  
  // Try to get precision from chart if available
  try {
    if (yAxis?.isInCandle() ?? true) {
      // For price-based overlays, use default precision
      precision = 2
    } else {
      // For indicator-based overlays, use default precision
      precision = 4
    }
  } catch (error) {
    // Fallback to default precision if any error occurs
    precision = 2
  }

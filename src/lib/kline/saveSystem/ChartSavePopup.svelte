<script lang="ts">
  ...
  onMount(() => {
    console.log('🎯 ChartSavePopup mounted with props:', {
      show,
      savedLayouts,
      activeSaveId,
      isLoading,
  ...
  function handleSaveAs() {
    // Close this popup and let parent handle the save as modal
    dispatch('saveAs');
    handleClose();
  }
  ...
  function getPopupStyle() {
    ...
    // Center the popup in the chart area
...
{#if show}
  ...
  >
    ...
    <div class="flex items-center justify-between p-4 border-b border-base-300/50">
      <h3 class="text-lg font-semibold text-base-content">Save Chart Layout</h3>
      <button
    ...
    <div class="p-4 border-b border-base-300/50">
      <div class="space-y-2">
        ...
        <button
          class="w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-base-200 rounded-md transition-colors duration-150"
    ...
    <!-- Current Layout Info -->
    {#if activeLayout}
      <div class="p-4 bg-primary/5 border-b border-base-300/50">
        <div class="flex items-center gap-2 text-sm">
          <div class="w-2 h-2 bg-primary rounded-full"></div>
          <span class="font-medium text-base-content">Active: {activeLayout.name}</span>
          {#if hasUnsavedChanges}
            <span class="text-xs text-warning">• Unsaved changes</span>
          {/if}
        </div>
        <div class="text-xs text-base-content/60 mt-1">
          Last saved: {formatDate(new Date(activeLayout.updatedAt).getTime())}
        </div>
      </div>
    {/if}
...
<style>
  /* Custom scrollbar for layouts list */
  .overflow-y-auto {

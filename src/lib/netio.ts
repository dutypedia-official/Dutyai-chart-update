...
const requestApi = async function(method: ApiType, url: string,
                                  ...
                                  body?: RequestInit["body"] | Record<string, unknown>): Promise<ApiResult> {
  const {token} = get(auth)
  try {
    const headers: Record<string, unknown> = {'X-Authorization': token}
    // @ts-ignore
    ...
    const data = rsp as Record<string, unknown>
...
export async function postApi(url: string, body: RequestInit["body"] | Record<string, unknown>,
